...
def test_named_interior_vertex_is_rejected(make_unit_square):
    ...
    bc = Conditions(Dirichlet(at_indices([interior]), 0))
    with pytest.raises(ValueError):
        bc.resolve(FunctionSpace(mesh, n_components=1))
...
def test_geometric_region_never_selects_interior_vertices(make_unit_square):
    ...
    bc = Conditions(Dirichlet(on_plane(0, 0.5), 0))
    ...
    fixed = bc.resolve(FunctionSpace(mesh, n_components=1)).fixed_idxs
...
def test_agreeing_overlapping_regions_are_fine(make_unit_square):
    ...
    resolved = bc.resolve(FunctionSpace(mesh, n_components=1))
...
def test_a_specification_is_a_frozen_tuple_of_conditions():
    ...
    bc = Conditions(pinned, Neumann(everywhere(), 0))
    assert [type(c) for c in bc] == [Dirichlet, Neumann]
    assert Conditions(*bc, pinned).items == (*bc.items, pinned)
