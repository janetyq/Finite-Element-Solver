...
def test_elastic_error_estimator_neumann_matching_traction_is_quiet():
    ...
    two endpoints' nodal `g`) -- the same nodal representation the solver
    itself uses to apply tractions (`fem/problem.py`'s `Traction`). Declaring
    top's traction therefore also lifts `g` on right and left away from zero
    at the shared corner, changing their residuals too; this is that
    corner-sharing worked through by hand rather than assumed away.
    """
    mesh = _unit_square_two_triangles()
    bc = BoundaryConditions()
