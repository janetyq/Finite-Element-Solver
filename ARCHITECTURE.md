...
one job. Anchored on symbol names rather than line numbers, since the type-hints branch is
moving underneath.
...
### `RefinementMesh` — a second mesh representation with a stringly-typed state machine

It shadows `vertices` / `elements` / `boundary`, maintains a parallel `Triangle` tree, and
syncs back through `update_mesh`. Triangle state is a **bare string** — `'red child'`,
`'green parent'`, `'gone'` — dispatched through `if/elif` chains with `assert False` on the
unknown branch. That is the pattern `AGENTS.md` explicitly rejects, and it is load-bearing
here: the red/green algorithm's correctness *is* those transitions. An `Enum` costs nothing
and makes the state machine checkable.

Related: `refine_red` returns `new_triangle_idxs`, and the `'green child'` path depends on
that return value — but the `'green parent'` path discards it, and `refine` never propagates
it. Whether that asymmetry is intentional is not clear from the code.

The debug `plot` method is broken: it calls `self.mesh.plot(title=…, show=False, linewidth=3)`
and expects an `ax` back, but `Mesh.plot(self)` takes no arguments and returns `None`. Both
that call and the `plotting_mesh.plot(ax=…, …)` at the end raise `TypeError`. It is
scaffolding that rotted when the plotter was decoupled — the commented-out `self.plot(...)`
calls throughout the file are its only callers.
...
| `Solution.get_values(name, iter_idx, mode)` | three-axis generality; the `mode` axis is unreachable and calls methods that do not exist |
| `Solver.solve_nonlinear_system` | general Newton hook, zero callers |
| `quadrature.py` | five rules, zero callers |
| `RefinementMesh.plot(title, edge, main_idx, green_idx, red_idx, triangle_idxs)` | six optional debug params, raises `TypeError` |
