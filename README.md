...
| **Equation** | physics | the PDE and its constants: `Projection`, `Poisson`, `Heat`, `Wave`, `LinearElastic`, or `FiniteStrainElastic` | |
| | material law (optional) | `FiniteStrainElastic(law=)` takes `StVenantKirchhoff` or `NeohookeanEnergyDensity` | St Venant-Kirchhoff |
| | element | the equation builds the problem with `.problem(mesh, conditions, element_type=)`, where the element is `LinearTriangleElement`, `LinearTetrahedralElement`, `QuadraticTriangleElement`, or `IsoparametricTriangleElement` | linear, by the mesh's dimension |
