...
| | backend | how each linear system is solved, held by the problem (`problem.with_backend(IterativeBackend())`): `DirectBackend` (the default), `IterativeBackend`, or `MinresBackend`; a linear problem factors once and holds the result for every solve | `fem/algebra/backends.py`, `fem/algebra/system.py` |
...
- PDEs: L2 projection, Poisson, variable-coefficient diffusion, heat, wave, and two
  ...
  hyperelastic law, St Venant-Kirchhoff by default); 2D elasticity is plane strain by
  default or plane stress by choice; one-way thermoelasticity, a temperature field
  driving a thermal strain in the small-strain model
