...
| | what | boundary conditions `Dirichlet`, `Neumann`, `Robin` on a region, a volume `Source`, `PointLoad`s, and the `Initial(u0, v0=)` state a solve starts from, collected into one `Conditions(...)`; with no `Initial` the solve starts at rest |
...
| | backend | how each linear system is solved, via `backend=`: `DirectBackend` (the default), `IterativeBackend`, or `MinresBackend` |
