...
class Solution:
    ...
    def plot_colored(self, name, idx=None, title=None, contour=0, ax=None, show=True, cbar=None, cbar_lim=None, cbar_label=None):
        if name not in self.values:
            raise ValueError(f'{name} not found in solution')
    ...
    def plot_animation(self, name, title='Animation', cbar_label='Value', fixed_cbar=False, surface=False, savefile=None, fps=15):
        if name not in self.values:
            raise ValueError(f'{name} not found in solution')
